...
def visualize(sess, dcgan, config, option, sample_dir='samples'):
  ...
  if option == 0:
    ...
    for idx in xrange(config.img_num):
      print(" [*] %d" % idx)
      z_sample = np.random.uniform(-1, 1, size=(config.batch_size , dcgan.z_dim))
      for kdx, z in enumerate(z_sample):
        #z[idx] = values[kdx]
        z[idx%100] = values[kdx]
      ...
      save_images(samples, [image_frame_dim, image_frame_dim], os.path.join((sample_dir+'/testing'), 'test_arange_%s.png' % (idx)))
